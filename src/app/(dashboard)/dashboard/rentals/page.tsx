...
const getLeaseLabel = (property: RentalPropertyItem) => {
  const term = property.rentalTerms?.leaseTerm
  if (!term) return 'N/A'

  const labels = [
    term.monthToMonth ? 'Month-to-month' : null,
    term.sixMonths ? '6 months' : null,
    term.twelveMonths ? '12 months' : null,
    term.other ? term.otherText || 'Other' : null,
  ].filter(Boolean)
...
const getStatusBadgeClassName = (status?: string) => {
  switch ((status || '').toLowerCase()) {
    ...
    case 'archived':
      return 'border-[#F3D6AA] bg-[#FFF7ED] text-[#B54708]'
    case 'disabled':
...
function DashboardRentalsPageContent() {
  ...
  const createNextUrl = (updates: Record<string, string | number | null | undefined>) => {
    ...
    return `/dashboard/rentals?${params.toString()}`
  }

  const rentalsQuery = useQuery({
    queryKey: ['dashboard-rentals', token, searchParams.toString()],
    queryFn: () =>
      fetchJson<RentalsResponse>(
        `/rental-properties?listingType=rent&page=${page}&limit=${limit}${
  ...
  const properties = rentalsQuery.data?.data.properties || []
  const paginationInfo = rentalsQuery.data?.data.paginationInfo
            ...
            <div className="space-y-8 bg-[#f8fafc] px-4 py-6 md:px-8 lg:px-12 xl:px-24 xl:py-8 2xl:px-32">
              ...
              <div className="rounded-[12px] border border-[#E5E7EB] bg-white p-5 shadow-[0_4px_12px_rgba(16,24,40,0.08)]">
                ...
                <div className="space-y-4">
                  {Array.from({ length: 6 }).map((_, index) => (
                    <div key={index} className="grid gap-4 border-b border-[#EEF2F6] pb-4 md:grid-cols-[1.6fr_1fr_1.3fr_1fr_1fr_1fr_0.9fr_0.8fr]">
        ...
        <section className="overflow-hidden border border-[#e7eaef] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.08)]">
          ...
          <div className="border-y border-[#eef2f6] bg-white px-5 py-4 md:px-8 xl:px-16">
            <nav className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
              {dashboardNavLinks.map(link => {
                ...
                const isActive = link.href === '/dashboard/rentals'
          ...
          <div className="space-y-8 bg-[#f8fafc] px-4 py-6 md:px-8 lg:px-12 xl:px-24 xl:py-8 2xl:px-32">
            <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-[22px] font-bold text-[#111827]">Rentals management</h2>
                <p className="mt-2 text-sm text-[#667085]">
                  Manage your {dashboardRole === 'AGENT' ? 'client' : 'rental'} listings in one place.
                </p>
              </div>
              <Link
                href="/list-property"
            ...
            <section className="grid gap-4 rounded-[12px] border border-[#E5E7EB] bg-white p-5 shadow-[0_4px_12px_rgba(16,24,40,0.08)] md:grid-cols-2 xl:grid-cols-4">
              ...
              <label className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    router.push(
                      createNextUrl({ island: value === ALL_FILTER_VALUE ? null : value, page: 1 }),
                    )
                  }
                >
              ...
              <label className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    router.push(
                      createNextUrl({ type: value === ALL_FILTER_VALUE ? null : value, page: 1 }),
                    )
                  }
                >
                  <SelectTrigger className="h-11 w-full rounded-[8px] border-[#D9DBE3] bg-white px-3 text-sm text-[#111827]">
              ...
              <label className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    router.push(
                      createNextUrl({ status: value === ALL_FILTER_VALUE ? null : value, page: 1 }),
                    )
                  }
                >
                  <SelectTrigger className="h-11 w-full rounded-[8px] border-[#D9DBE3] bg-white px-3 text-sm text-[#111827]">
            ...
            <section className="rounded-[12px] border border-[#E5E7EB] bg-white p-5 shadow-[0_4px_12px_rgba(16,24,40,0.08)]">
              <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="text-lg font-bold text-[#111827]">Your rentals</h3>
              ...
              {rentalsQuery.isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-[12px] border border-[#FECACA] bg-[#FEF2F2] px-6 py-12 text-center">
                  ...
                  <h4 className="mt-4 text-xl font-bold text-[#111827]">Couldn&apos;t load rentals</h4>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-b border-[#EEF2F6] hover:bg-transparent">
                        ...
                        <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-[#98A2B3]">Rent</TableHead>
