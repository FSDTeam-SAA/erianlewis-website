...
export function ListingNavbar() {
  ...
  const canListProperty =
    Boolean(session) && (role === 'LANDLORD' || role === 'AGENT')

  const isRentals = pathname?.includes('/rentals')
  const isBuy = pathname?.includes('/buy')
  const isListProperty = pathname?.includes('/list-property')
          ...
          {session ? (
            <div className="hidden items-center gap-5 lg:flex">
              ...
              {canListProperty ? (
                <Link
                  href={ownerCtaHref}
                  className={`${getNavLinkClassName('/list-property', isListProperty)} tracking-wide`}
                >
                  List Your Property
                </Link>
              ) : null}
              ...
              <span className="text-[15px] text-gray-500">
                ...
                <span className="font-semibold text-gray-900">
                  {session.user?.name || 'Rifat Hossain'}
                </span>
              </span>
              <CurrencySelector
              ...
              />
              <Link href="/account" className={getNavLinkClassName('/account')}>
                My Account
              </Link>
              <button
                type="button"
